...
def group_data(data, degree=3, hash=hash):
    ...
    m,n = data.shape
    new_data = [
        [hash(tuple(v)) for v in data[:, indicies]]
        for indicies in combinations(list(range(n)), degree)
    ]
...
def OneHotEncoder(data, keymap=None):
    ...
    if keymap is None:
        ...
        for col in data.T:
            ...
            keymap.append({key: i for i, key in enumerate(uniques)})
...
def create_test_submission(filename, prediction):
    content = ['id,ACTION']
    content.extend('%i,%f' %(i+1,p) for i, p in enumerate(prediction))
...
def main(train='train.csv', test='test.csv', submit='logistic_pred.csv'):    
    ...
    while len(score_hist) < 2 or score_hist[-1][0] > score_hist[-2][0]:
        ...
        print(f"Current features: {sorted(list(good_features))}")
